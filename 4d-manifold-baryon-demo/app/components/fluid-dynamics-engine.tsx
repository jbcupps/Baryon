...
export class FluidDynamicsEngine {
  ...
  generateFluidKleinBottleSurface(params: KleinBottleSurfaceParams): THREE.Vector3 {
    ...
    // Independent wiggle for each sub-4D Manifold
  ...
  applyTimeVaryingWarping(
    ...
    time: number,
    params: FluidDynamicsParameters,
    sub4DManifoldIndex: number
  ): Float32Array {
    const warpedVertices = new Float32Array(vertices.length);
    const vertexCount = vertices.length / 3;
    
    // Independent noise offset for each sub-4D Manifold
    const noiseOffset = new THREE.Vector3(
      sub4DManifoldIndex * 100,
      sub4DManifoldIndex * 200,
      sub4DManifoldIndex * 300
    );
    ...
    for (let i = 0; i < vertexCount; i++) {
      ...
      // Independent wiggle for each sub-4D Manifold
      const wigglePhase = sub4DManifoldIndex * Math.PI / 3;
