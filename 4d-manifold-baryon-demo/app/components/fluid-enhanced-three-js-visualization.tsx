...
const FluidEnhancedThreeJSVisualization: React.FC<FluidEnhancedThreeJSVisualizationProps> = ({
  ...
  // Add ref to track last progress value to prevent unnecessary updates
  ...
  // Update lastProgressRef when progress changes
  useEffect(() => {
  ...
  const generateFluidKleinBottleSurface = (
    ...
    const surfaceParams: KleinBottleSurfaceParams = {
      ...
      noiseOffset: new THREE.Vector3(sub4DManifoldIndex * 100, sub4DManifoldIndex * 200, sub4DManifoldIndex * 50),
      wigglePhase: sub4DManifoldIndex * Math.PI / 3
    };
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (isPlayingRef.current) {
        ...
        // Only call onProgressChange if the progress has actually changed
        if (Math.abs(newProgress - lastProgressRef.current) > 0.001) {
          onProgressChange(newProgress);
          lastProgressRef.current = newProgress;
        }
      } else {
        // When paused, synchronize frameRef with the current progress
        const targetFrame = Math.floor(progress * 100);
  ...
  useEffect(() => {
    ...
    showFluidDynamics, fluidDynamicsParams, isPlaying
  ]);
